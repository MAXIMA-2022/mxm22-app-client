...
const MapsItems: NextPage = ({ dataMAPS }: any) => {
  ...
  const MapsData = [
    {
      bgImg: `/MapsTents/${mapsID}_pc.png`,
      bgImg2: `/MapsTents/${mapsID}_mobile.jpg`,
      title: capitalize(mapsID),
      details: `Kalau kamu tertarik dengan APA YA. ${capitalize(mapsID)} Camp cocok menjadi tempat singgah kamu.`,
    },
  ];
    ...
    return (
      <Box>
        ...
        <Box display={["block", "block", "block", "none", "none"]}>
          <Center m={["14vh 5vh", "14vh 5vh", "4rem 8rem", "14vh 5vh", "14vh 5vh"]}>
  ...
  const DetailButton = () => {
    return (
      <Box position={"absolute"} left={0} bottom={0} right={0} mb={["13vh", "13vh", "5vh", "5vh", "5vh"]}>
        <Center>
  ...
  return (
    <Layout>
      <Flex minH={"100vh"} bgImage={[MapsData[0].bgImg2, MapsData[0].bgImg2, MapsData[0].bgImg, MapsData[0].bgImg, MapsData[0].bgImg]} bgPosition={"center"} bgSize={"cover"} bgRepeat={"no-repeat"}>
        <Box w={"full"}>
          <BubbleInfo />
          <DetailButton />
        </Box>
